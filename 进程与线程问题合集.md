...
JAVA实现线程有两种方式，一种是继承Thread类，另一种是实现Runnable接口。

**1. 继承Thread类**

首先，需要关注其中的几个方法
- Thread()：这是Thread类的一个构造方法，没有任何参数。  

- void start()：此方法是启动线程的方法，是线程类中最核心的方法。当调用这个方法之后，它就会启动线程。

- void run()：此方法是Thread类中的一个普通方法，它的特殊之处仅仅在于start()方法会将它作为线程的起点  

首先来创建一个Thread类的子类，并且覆盖Thread类中的run()方法。
...
JAVA的类只能是单继承，也就是说，如果一个类为了使用线程而继承了Thread类，那么它就不能再继承别的类了。所以此处是一种脱离继承来使用线程的方法。这个方法的核心就是Runnable接口。

Myrunnable实现了Runnable接口，其run()方法就是线程回去执行的方法。
```
package usethread;
...
```
下面是例程，按照步骤分别创建MyRunnable类的实例，调用Thread相应的构造方法，最后启动线程。
```
package usethread;
...
```
从本质上讲，无论是使用继承还是使用Runnable接口，其目的都是一样的。让线程执行我们写的一段代码。
